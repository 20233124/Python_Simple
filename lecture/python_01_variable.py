...
# - Python 정수형: int (옛날에 비해 메모리가 늘어나서 통일)
# 1) 문자열(String): "Hello", 'Hi'
# 2) 정수형(int): 3, -1, 0
# 3) 실수형(float): 3.14, 0.0, -2.2
# 4) 논리형(boolean): True, False (T,F 대문자로 써야함)

# 설명: 동일한 Type에서 다향한 종류의 자료형을 사용하는 이유?
...
# print(int(str_float_d))  # Process finished with exit code 1
...
student_name = ""   # 적극 권장! (아무것도 없는 문자열)

# "Null Pointer Exception" 프로그램이 꺼져버림

# 기본 자료형(Primitive Type): 변수에 값이 그대로 저장
#  - int num = 4;
# 객체 자료형(Reference Type): 변수에 값이 위치한 "주소"가 저장 # Null 가르킬 주소에 값이 없으면 Null
#  - String name = "10";
...
#  - 변수 생성 및 초기화
#   문법: num = 5         # 5라는 값을 num에 저장한다
#    * num: "num" 변수 생성
#    * 대입연산자(=): 우측의 값을 좌측에 저장
#    * 동등연산자(==): Equal
#    * 초기화: 초기 변수를 생성하면 쓰레기 파일들이 존재
#             변수에 값을 대입하면 공간이 초기화 되고 값만 저장!
#   aa.txt를 지우면 파일이 있던 저장공간을 오픈하고 aa.txt은 아직있음, 새로운 파일을 만들어야 지워짐
